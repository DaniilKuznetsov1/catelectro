...
const emit = defineEmits<{
  (e:'resdataedittov', succesdata:any):void;
}>();
...
watch(() => props.tovar, (newVal) => {
  if (newVal) {
    console.log('watch props.tovar');
    form.id = newVal.id;
    form.name = newVal.name;
    form.description = newVal.description;
    form.photo = newVal.photo;
    form.price = newVal.price;
    form.valprice = newVal.valprice;
    form.category_id = newVal.category_id;
    imageUrl.value = newVal.photo;
    //console.log(form);
  }
}, {deep: true});
...
const submit = () => {
    console.log(form.id);
    form.post(route('products.update', {tovar: form.id}), { //  /categorys/update
        forceFormData: true,
        onFinish: () => {
          form.reset('name');
        ...
        onSuccess: (page) => {
          emit('resdataedittov', page.props.activeremount); 
        }
    });
};
